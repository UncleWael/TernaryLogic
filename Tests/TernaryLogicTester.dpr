...
{$ENDIF }
  DUnitX.TestFramework, UnitTests in 'UnitTests.pas',
  TernaryLogic in '..\Source\TernaryLogic.pas';

{ keep comment here to protect the following conditional from being removed by the IDE when adding a unit }
{$IFNDEF TESTINSIGHT}

var runner: ITestRunner; results: IRunResults; logger: ITestLogger;
  nunitLogger: ITestLogger; StartTime: TDateTime;
{$ENDIF}
...
{$IFNDEF CI}
    ...
    if TDUnitX.Options.ExitBehavior = TDUnitXExitBehavior.Pause then begin
      System.Writeln(format('Tests Duration: %.3d msec',
